...
async function onAddOrders(ids: string[]) {
  ...
  const moved = pallets.addOrders(pallet.value.id, ids)
  if (!moved) return warn('No matching rows in pallet data. Check that order IDs equal one of Work/Barcode/Ident/Position.')
  success(`Added ${moved} line(s)`)
}
...
function doRemoveSelected() {
  ...
  const removed = pallets.removeOrders(p.id, ids)
  setSelected(new Set())
  success(`Removed ${removed} line(s)`)
}
...
function doMarkShipped() { const p = pallet.value; if (!p) return; pallets.markShipped(p.id); success('Pallet marked as shipped') }
function onReopen() { const p = pallet.value; if (!p) return; pallets.reopen(p.id); success('Pallet reopened') }
function openOrderDetail(id: string) { router.push({ name: 'order-detail', params: { id } }) }
</script>
  ...
  <section class="space-y-4">
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center gap-2">
        <button class="btn bg-red-600 hover:bg-red-700 text-white" :disabled="!pallet || pallet.status==='Shipped' || selectedIds.size===0" title="Remove selected from pallet" @click="onRemoveSelected">
          Remove Selected ({{ selectedIds.size }})
        </button>
        <button v-if="pallet?.status !== 'Shipped'" class="btn btn-primary" :disabled="!pallet || palletOrders.length===0 || overWeight" :title="overWeight ? 'Over max weight' : 'Mark shipped'" @click="onMarkShipped">
          Mark Shipped
        </button>
        <button v-else class="btn btn-ghost" @click="onReopen" title="Reopen pallet">Reopen</button>
      </div>
    </div>

    <PalletHeaderInfo v-if="pallet" :pallet="pallet" :orders-count="palletOrders.length" :total-weight="totalWeight" :max-weight="maxWeight" />
    ...
    <div class="flex items-center gap-3">
      ...
      <div class="text-sm" :class="overWeight ? 'text-rose-600 font-medium' : 'text-slate-500'">
        {{ totalWeight.toFixed(2) }} / {{ maxWeight.toFixed(2) }} kg <span v-if="overWeight">– Over capacity</span>
      </div>
    </div>
    ...
    <div class="card p-0">
        ...
        <div class="max-h-[65vh] overflow-auto">
          <table class="table table-fixed w-full">
            ...
            <tbody v-if="!loading && palletOrders.length>0">
              <tr v-for="(r, i) in palletOrders" :key="r.orderId" :class="['tr row-bg', i % 2 === 0 ? 'zebra-odd' : 'zebra-even', selectedIds.has(r.orderId) && 'is-selected']">
                <td class="td"><input type="checkbox" :checked="selectedIds.has(r.orderId)" @change="toggleRow(r.orderId)" /></td>
                ...
                <td class="td"><span :class="r.status === 'Pending' ? 'badge-amber' : 'badge-green'">{{ r.status }}</span></td>
                <td class="td">{{ r.transporter || '—' }}</td>
                <td class="td">{{ r.parcelNo || '—' }}</td>
                <td class="td">{{ r.deliveryDate || '—' }}</td>
                <td class="td sticky right-0 border-l border-slate-200">
              ...
              <tr>
                <td class="td p-6 text-center" colspan="8">
                  <EmptyState title="No orders in this pallet" description="Add pending orders to start packing.">
                    <template #actions>
                      <button class="btn btn-primary" :disabled="pallet?.status === 'Shipped'" @click="showAddDialog = true">+ Add Orders</button>
                    </template>
                  </EmptyState>
                </td>
              </tr>
      ...
      <div class="card-footer flex items-center gap-3">
        <div class="text-sm text-slate-600">{{ palletOrders.length }} orders · {{ totalWeight.toFixed(2) }} kg</div>
        <div class="ml-auto text-sm text-slate-500">Use Ctrl+A to select all · Delete to remove</div>
      </div>
    </div>

    <AddOrdersToPalletDialog v-model="showAddDialog" :pallet-id="palletId" :existing-ids="pallet?.orderIds ?? []" @add="onAddOrders" />
    <ConfirmDialog v-model="showConfirmShip" title="Mark pallet as shipped" :message="`Ship pallet #${palletId}?`" confirm-text="Ship" cancel-text="Cancel" @confirm="doMarkShipped" />
    <ConfirmDialog v-model="showConfirmRemove" title="Remove orders" :message="`Remove ${selectedIds.size} orders from pallet #${palletId}?`" confirm-text="Remove" cancel-text="Cancel" @confirm="doRemoveSelected" />
  </section>
</template>
